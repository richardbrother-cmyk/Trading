...
```bash
# 1) Backtest de 2 años sobre SPY, QQQ, AAPL, MSFT, NVDA (datos reales de Yahoo)
...
- **Tamaño**: `equity × riesgo_por_operación / (precio × stop_loss)`, acotado al 25 % del
  equity por posición y al efectivo disponible. Máximo 5 posiciones.
...
Es decir: la estrategia base es mediocre. Ese es el punto de partida del experimento;
la iteración (otros parámetros, otras señales, otros universos) se hace con `backtest`
antes de cambiar lo que opera en `run`.
